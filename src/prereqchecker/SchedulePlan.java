...
public class SchedulePlan {
    ...
    String schedule1 = args[1];
    String output1 = args[2];

    Digraph di = new Digraph(input1);

    StdIn.setFile(schedule1);
    String target = StdIn.readString();
    int num = StdIn.readInt();


    ArrayList<String> take = new ArrayList<String>();



    for(int i=0; i<num; ++i){
        take.add(StdIn.readString());
    }

    ArrayList<String> a = di.needToTakeOutput(take, target);
    Digraph re = new Digraph(a, di);
    re.setSemester2(output1);
    }
}
